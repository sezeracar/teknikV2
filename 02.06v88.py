    ...
    if df.empty or "Durum" not in df.columns:
        ...
        try:
            ...
            if not df_bp.empty and "sonraki_bakim_tarihi" in df_bp.columns:
                ...
                yak_bp  = df_bp[(df_bp["_snr"] >= date.today()) & (df_bp["_snr"] <= date.today()+timedelta(days=7)) & (df_bp["durum"]=="Bekliyor")]
                if not gec_bp.empty:
...
with tab_bakim:
    if not giris_gerektir("Teknisyen"):
        ...
        # Gecikmiş bakımları güncelle
        bakim_durumu_guncelle()
            ...
            with st.form("bakim_ekle_formu"):
                ...
                if submit_bakim:
                    ...
                    if ok:
                        ...
                        st.error("❌ Bakım planı kaydedilemedi.")
