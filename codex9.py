    ...
    if not giris_gerektir("Teknisyen"):
        ...
        if acik_k.empty:
            ...
            with st.form("kapat_formu"):
                ...
                with col_k2:
                    mudahale_bas = st.time_input(
                        "🕐 Müdahaleye Başlama Saati *",
                        value=datetime.now().replace(second=0, microsecond=0).time(),
                        help="Teknisyenin makinenin başına geçtiği saat"
                    )
                with col_k3:
                    mudahale_bit = st.time_input(
                        "🕑 Arıza Giderilme Saati *",
                        value=datetime.now().replace(second=0, microsecond=0).time(),
                        help="Makinenin tekrar devreye alındığı saat"
                    )

                bas_dt = datetime.combine(date.today(), mudahale_bas)
                bit_dt = datetime.combine(date.today(), mudahale_bit)
                if bit_dt < bas_dt:
                ...
                if submit_kapat:
                    if not mudahale_eden.strip():
                        ...
                        mudahale_aralik = f"{mudahale_bas.strftime('%H:%M')} - {mudahale_bit.strftime('%H:%M')}"
