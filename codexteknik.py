...
with st.sidebar:
    ...
    <div style="font-size:12px;color:#64748b;padding-left:4px;line-height:1.8;">
      ...
      🕐 {datetime.now().strftime("%H:%M")} (Türkiye)<br>
      🏭 Tesis: Gölcük Fabrikası<br>
      📋 Sistem: CMMS v2.0
    </div>
    """, unsafe_allow_html=True)
    ...
    if len(df) == 0:
        ...
        with col_g1:
            ...
            if acik > 0:
                ...
                onc_say = acik_df["Öncelik"].value_counts().reset_index()
                onc_say.columns = ["Öncelik","Adet"]
                renk_map = {k: v["renk"] for k, v in ARIZA_ONCELIKLERI.items()}
                fig = px.pie(
                    onc_say, values="Adet", names="Öncelik",
                    color="Öncelik", color_discrete_map=renk_map,
                    hole=0.55
                )
                fig.update_layout(
        ...
        with col_g2:
            ...
            mak_say = df["Makine"].value_counts().head(8).reset_index()
            mak_say.columns = ["Makine","Adet"]
            fig2 = px.bar(
                mak_say, x="Adet", y="Makine", orientation="h",
                color="Adet", color_continuous_scale=["#1e3a5f","#3b82f6","#93c5fd"]
            )
            fig2.update_layout(
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                ...
                xaxis=dict(gridcolor="rgba(99,179,237,0.08)", color="#475569"),
                yaxis=dict(gridcolor="rgba(0,0,0,0)", color="#94a3b8"),
            )
        ...
        try:
            ...
            gunluk  = df_trend.groupby("Tarih").size().reset_index(name="Adet")
            if len(gunluk) > 0:
                fig3 = go.Figure()
                fig3.add_trace(go.Scatter(
                    x=gunluk["Tarih"], y=gunluk["Adet"],
                    mode="lines+markers",
                    line=dict(color="#3b82f6", width=2.5),
                    marker=dict(color="#93c5fd", size=7, line=dict(color="#1d4ed8",width=1.5)),
                    fill="tozeroy",
                ...
                fig3.update_layout(
                    ...
                    xaxis=dict(gridcolor="rgba(99,179,237,0.08)", color="#475569",
                               showline=True, linecolor="rgba(99,179,237,0.15)"),
                    yaxis=dict(gridcolor="rgba(99,179,237,0.08)", color="#475569",
                               showline=True, linecolor="rgba(99,179,237,0.15)"),
                    height=220
                )
        ...
        if submit_yeni:
            if not bildiren.strip():
                ...
                yeni = {
                    ...
                    "Bildirim Saati": bildirim_saat.strftime("%H:%M"),
    ...
    if not giris_gerektir("Teknisyen"):
        ...
        if acik_k.empty:
            ...
            with st.form("kapat_formu"):
                ...
                df_stok_k = pd.read_csv(STOK_FILE)
                ...
                if submit_kapat:
                    if not cozum_aciklama.strip():
                        ...
                        if "—" not in malzeme_secim and malzeme_adet > 0:
                            ...
                            malzeme_metni = f"{malzeme_secim} × {malzeme_adet}"

                        toplam_maliyet = isguc_maliyet + malzeme_maliyet
                        sla_s = sla_hesapla(talep["Öncelik"], talep["Açılış Tarihi"], kapatma_zaman)

                        df_k2 = pd.read_csv(DB_FILE)
                        mask  = df_k2["Talep No"] == secilen_no
                        df_k2.loc[mask, "Durum"]              = "Kapalı"
                        df_k2.loc[mask, "Kapatma Tarihi"]     = kapatma_zaman
                        df_k2.loc[mask, "Müdahale Eden"]      = mudahale_eden
                        df_k2.loc[mask, "İlk Müdahale Saati"] = mudahale_saat
                        df_k2.loc[mask, "Çözüm Süresi (Dk)"] = cozum_suresi
                        df_k2.loc[mask, "SLA Durumu"]         = sla_s["durum"]
                        df_k2.loc[mask, "Çözüm Açıklaması"]  = cozum_aciklama.strip()
                        df_k2.loc[mask, "Kök Neden"]          = kok_neden
                        df_k2.loc[mask, "5 Neden Analizi"]    = neden_analizi
                        df_k2.loc[mask, "Kaizen Önerisi"]     = kaizen
                        df_k2.loc[mask, "Kullanılan Malzemeler"]   = malzeme_metni
                        df_k2.loc[mask, "Malzeme Maliyeti (TL)"]   = malzeme_maliyet
                        df_k2.loc[mask, "İş Gücü Maliyeti (TL)"]  = isguc_maliyet
                        df_k2.loc[mask, "Toplam Maliyet (TL)"]     = toplam_maliyet
                        df_k2.loc[mask, "Kapatma Onayı"]           = kapatma_onayi
...
with tab_stok:
    if not giris_gerektir("Teknisyen"):
        ...
        df_st["Doluluk %"] = (df_st["Stok Miktarı"] / df_st.get("Maksimum Stok", df_st["Stok Miktarı"]).replace(0,1) * 100).round(1)
        fig_stok = go.Figure()
        renkler = ["#f87171" if r<=20 else "#fbbf24" if r<=50 else "#4ade80" for r in df_st["Doluluk %"]]
        fig_stok.add_trace(go.Bar(
            x=df_st["Doluluk %"], y=df_st["Malzeme Adı"],
            orientation="h",
            marker=dict(color=renkler, line=dict(color="rgba(0,0,0,0)", width=0)),
            text=[f"%{r}" for r in df_st["Doluluk %"]],
            textposition="inside", textfont=dict(color="#0f172a", size=11, family="Inter"),
        ))
        fig_stok.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#94a3b8", size=12, family="Inter"),
            margin=dict(t=10, b=10, l=10, r=10),
            xaxis=dict(range=[0,110], gridcolor="rgba(99,179,237,0.06)", color="#475569",
                       ticksuffix="%", showgrid=True),
            yaxis=dict(gridcolor="rgba(0,0,0,0)", color="#94a3b8", autorange="reversed"),
            height=280, title=dict(text="Stok Doluluk Oranları", font=dict(color="#94a3b8", size=13))
        )
        ...
        with col_form:
            ...
            with st.form("stok_guncelle"):
                ...
                if guncelle_btn:
                    df_st2 = pd.read_csv(STOK_FILE)
                    mask_s = df_st2["Malzeme Adı"] == secilen_mal
                    mevcut = df_st2.loc[mask_s, "Stok Miktarı"].values[0]
                    ...
                    df_st2.loc[mask_s, "Stok Miktarı"]   = yeni_m
