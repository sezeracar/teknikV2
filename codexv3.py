            ...
            with st.form("kapat_formu"):
                ...
                if submit_kapat:
                    if not cozum_aciklama.strip():
                        ...
                        # dtype uyumsuzluğunu önlemek için CSV'yi tamamen string olarak oku
                        df_k2 = pd.read_csv(DB_FILE, dtype=str)
                        guncelle = {
                            "Durum":                  "Kapalı",
                            "Kapatma Tarihi":         str(kapatma_zaman),
                            "Müdahale Eden":          str(mudahale_eden),
                            "İlk Müdahale Saati":     str(mudahale_saat),
                            "Çözüm Süresi (Dk)":      str(cozum_suresi),
